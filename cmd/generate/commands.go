...
var cmdSkip = map[string]string{
	...
	"PatchOrderByOrderID": "hand-written: advanced-instructions JSON unmarshal",

	"PostWatchlist":             "hand-written: POST body + comma-split symbols",
	"UpdateWatchlistByID":       "hand-written: PATCH body + comma-split symbols",
	"UpdateWatchlistByName":     "hand-written: PATCH body + --new-name + comma-split symbols",
	"AddAssetToWatchlist":       "hand-written: POST body from flag",
	"AddAssetToWatchlistByName": "hand-written: POST body from flag + params",

	"StockBarSingle":         "hand-written: fetchPaginated + unified routing",
	"StockQuoteSingle":       "hand-written: fetchPaginated + unified routing",
	"StockTradeSingle":       "hand-written: fetchPaginated + unified routing",
	"StockSnapshotSingle":    "hand-written: unified routing (url.Values)",
	"StockLatestTradeSingle": "hand-written: unified routing (url.Values)",
	"StockLatestQuoteSingle": "hand-written: unified routing (url.Values)",
	"StockLatestBarSingle":   "hand-written: unified routing (url.Values)",
...
func genCommands(allEndpoints []*endpointInfo, allSchemas []*schemaInfo) string {
	...
	for _, key := range parentKeys {
		...
		if len(pdef.aliases) > 0 {
			fmt.Fprintf(&buf, "\tAliases: []string{")
	...
	for _, opID := range opIDs {
		...
		emitCommand(&buf, opID, def, ep, allSchemas)
	}

	// Emit init() for subcommand wiring within groups
	emitInit(&buf, opIDs, allEndpoints)
