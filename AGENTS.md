...
Query param structs are populated via generated `xParamsFromFlags(cmd)` functions. Each uses `flags.Lookup()` guards so excluded flags are safely skipped. RunE functions call `FromFlags`, then apply custom defaults or overrides:
